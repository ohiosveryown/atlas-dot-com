<template>
  <div class="wrapper">
    <div class="best-value tac uc inter">
      <span>best value</span>
    </div>
    <ul class="cards">
      <li class="card">
        <img
          class="icon"
          src="https://atlas-www-assets.s3.amazonaws.com/icon--startup.png"
          alt=""
        />
        <header>
          <h3 class="inter">Startup</h3>
          <div class="pricing-card-price">
            <h4 class="inter">$30</h4>
            <span>per seat, per month – min 5 seats</span>
          </div>
        </header>
        <ul>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Ticketing</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Chat, Email, SMS</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Customer Timeline</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Session Recording</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Bug Tracking</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Search</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Integrations, API, Webhooks</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>24 hour Support SLA</span>
          </li>
        </ul>

        <footer>
          <button class="inter tertiary">Get the Startup Plan</button>
        </footer>
      </li>
      <li class="card">
        <img
          class="icon"
          src="https://atlas-www-assets.s3.amazonaws.com/icon--growth.png"
          alt=""
        />
        <header>
          <h3 class="inter">Growth</h3>
          <div class="pricing-card-price">
            <h4 class="inter">$50</h4>
            <span>per seat, per month</span>
          </div>
        </header>
        <ul class="pricing-card-list">
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Startup Plan Plus:</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>CSAT</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Custom Events</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Custom Inboxes</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Ticket Routing</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Help Center</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Chatbot Workflows</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Teams</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Slack Support</span>
          </li>
        </ul>

        <footer>
          <button class="inter button-primary">🪴 Get the Growth Plan 🪴</button>
        </footer>
      </li>
      <li class="card">
        <img
          class="icon"
          src="https://atlas-www-assets.s3.amazonaws.com/icon--enterprise.png"
          alt=""
        />
        <header>
          <h3 class="inter">Enterprise</h3>
          <div class="pricing-card-price">
            <h4 class="inter">$80</h4>
            <span>per seat, per month</span>
          </div>
        </header>

        <ul class="pricing-card-list">
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Growth Plan Plus:</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Gated Help Centers</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Automations</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Suggested Response</span>
          </li>
          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>Smart Routing</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>SAML</span>
          </li>

          <li>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="13"
              fill="none"
            >
              <path
                fill="#EBE9E3"
                fill-rule="evenodd"
                d="M2.066 5.306a.577.577 0 0 0-.246-.055H.592c-.118 0-.183.131-.111.22-.072-.089-.007-.22.11-.22h1.23a.577.577 0 0 1 .245.055Zm3.67 4.423L12.985.826a.566.566 0 0 1 .441-.208h1.23-1.23a.566.566 0 0 0-.44.208L5.736 9.73Zm-.389 1.712a.57.57 0 0 0 0 0ZM15.116 1.11l.001-.002c.312-.38.022-.926-.463-.926h-1.229c-.308 0-.603.137-.795.375v.001L5.736 9.025 2.615 5.189a1.007 1.007 0 0 0-.353-.275 1.041 1.041 0 0 0-.442-.1H.592c-.493 0-.77.552-.464.927l4.814 5.915a1.034 1.034 0 0 0 1.59.002L15.116 1.11Z"
                clip-rule="evenodd"
              />
            </svg>
            <span>White Glove Support</span>
          </li>
        </ul>

        <footer>
          <button class="inter tertiary">🌳 Get the Enterprise Plan 🌳</button>
        </footer>
      </li>
    </ul>
  </div>
</template>

<style lang="scss" scoped>
  @import "~static/style/grid.scss";

  .best-value {
    display: none;
    @include breakpoint(md) {
      display: inherit;
      margin: 0 auto 1.8rem;
      width: max-content;
      border-radius: 5px;
      padding: 0.6rem 1.6rem;
      background: rgba(255, 255, 255, 0.1);
    }
    span {
      font-size: 1.4rem;
      background: linear-gradient(106.92deg, #b6bffe 0%, #ff56b1 98.96%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
      text-fill-color: transparent;
    }
  }

  .cards {
    @include breakpoint(md) {
      display: flex;
      justify-content: space-between;
    }
  }

  .card {
    height: fit-content;
    position: relative;
    border-radius: 20px;
    margin-bottom: 4rem;
    padding: 2rem;
    background: linear-gradient(
      135deg,
      rgba(58, 51, 73, 1) 2%,
      rgba(58, 51, 73, 0.22) 88%
    );
    box-shadow: 0px 100px 80px rgba(0, 0, 0, 0.75),
      0px 12px 10px rgba(0, 0, 0, 0.375), inset 0px 1px 1px #5c546c;
    overflow: hidden;
    @include breakpoint(md) {
      width: grid-width(3.65);
      margin-bottom: 0;
    }

    .pricing-card-price {
      @include breakpoint(md) {
        display: flex;
      }
    }

    .icon-label {
      display: flex;
      align-items: center;

      span {
        margin-left: 1.2rem;
        padding: 0.6rem 1.2rem;
        background: linear-gradient(106.92deg, #b6bffe 0%, #ff56b1 98.96%);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        background-clip: text;
        text-fill-color: transparent;
        text-transform: uppercase;
      }
    }

    h3 {
      padding: 0rem 0 0.2rem;
      font-size: 2.4rem;
      color: #c7c3cd;
    }

    h4 {
      font-size: 4rem;
      @include breakpoint(md) {
        margin-right: 1.2rem;
        font-size: 5.2rem;
        letter-spacing: -2px;
      }
    }

    span {
      margin-top: 1rem;
      max-width: 16ch;
      color: #c7c3cd;
      display: flex;
    }

    ul {
      margin: 2rem 0 3.2rem;
    }

    li {
      display: flex;
      flex-direction: row;
      align-items: center;
      margin: 1.2rem 0;
      font-size: 1.6rem;

      svg {
        opacity: 0.72;
      }

      span {
        color: #fff;
        margin: -0.1rem 0 0 1.4rem;
      }
    }

    footer {
      display: flex;
      width: 100%;
      justify-content: center;
    }
  }

  .card:nth-of-type(2) {
    box-shadow: 0px 100px 80px rgba(0, 0, 0, 0.75),
      0px 12px 10px rgba(0, 0, 0, 0.375), inset 0px 1px 1px #5c546c;
    @include breakpoint(md) {
      box-shadow: 0px 100px 80px rgba(0, 0, 0, 0.75),
        0px -4px 80px rgba(145, 101, 249, 0.5), inset 0px 1px 1px #5c546c;
    }
  }

  .icon {
    margin-bottom: 1.2rem;
    width: 5.6rem;

    @include breakpoint(md) {
      margin-bottom: 1.6rem;
    }
  }

  button {
    color: #fff;
    font-size: 1.4rem;
    position: relative;
    white-space: nowrap;
  }

  .button-primary {
    padding: 1.4rem 2.4rem 1.6rem;
    transition: filter 300ms ease;
  }

  .tertiary {
    background: rgba(255, 255, 255, 0);
    border-radius: 100px;
    padding: 1.4rem 2.4rem 1.6rem;
    border: 1px solid #591bdd;
    transition: background 300ms ease;
  }

  @media (pointer: fine) {
    .button-primary:hover {
      filter: hue-rotate(20deg);
    }

    .tertiary:hover {
      background: rgba(255, 255, 255, 0.05);
    }
  }
</style>

<script>
  export default {}
</script>
